...
#define TSIMD_DISPATCH_THIS_FILE "batch/dyn_dispatch_float/x86/float32/SSE2_float32.cpp" // this file
...
namespace tsimd
{
    namespace TSIMD_DYN_INSTRUCTION
    {
        TSIMD_DYN_FUNC_ATTR void kernel_dyn_impl(const float* TSIMD_RESTRICT arr, const size_t N, float* TSIMD_RESTRICT out_result) noexcept
        {
            ...
            // 测试SimdOp后端
            bool test = std::is_same_v<op, SimdOp<SimdInstruction::SSE2, float>>;
            EXPECT_TRUE(test);
