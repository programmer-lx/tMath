#define TSIMD_TEST_INTRINSIC SSE3
...
#define TSIMD_DISPATCH_THIS_FILE "batch/x86/float32/SSE3_float32.cpp" // this file
...
namespace tsimd
{
    namespace TSIMD_DYN_INSTRUCTION
    {
        TSIMD_DYN_FUNC_ATTR void kernel_dyn_impl(const float* TMATH_RESTRICT arr, const size_t N, float* TMATH_RESTRICT out_result) noexcept
        {
            ...
            // 测试SimdOp后端
            bool test = std::is_same_v<op, SimdOp<SimdInstruction::SSE3, float>>;
            EXPECT_TRUE(test);
            ...
            EXPECT_TRUE(cur_intrinsic == "\"\" __attribute__((target(\"sse3\")))");
#else
